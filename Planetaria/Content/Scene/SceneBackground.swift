...
class SceneBackground: Entity {
    ...
    required init() {
        ...
        Task {
            #if os(visionOS)
            guard let resource = try? await TextureResource(named: textureName) else { return }
            #elseif os(iOS) || os(macOS)
            var resource: TextureResource
            if #available(iOS 18, macOS 15, *) {
                guard let r = try? await TextureResource(named: textureName) else { return }
                resource = r
            } else {
                guard let r = try? TextureResource.load(named: textureName) else { return }
                resource = r
            }
            #endif
